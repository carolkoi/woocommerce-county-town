...
if(in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
    ...
    class WC_States_Places {
        ...
        public function  wc_states($states) {
            ...
            if (!empty( $allowed ) ) {
                foreach ($allowed as $code => $country) {
                    if (! isset( $states[$code] ) && file_exists($this->get_plugin_path() . '/counties/' . $code . '.php')) {
                        include($this->get_plugin_path() . '/counties/' . $code . '.php');
                    }
                }
            }
        ...
        public function load_country_places() {
            ...
            if ( $allowed ) {
                foreach ( $allowed as $code => $country ) {
                    if ( ! isset( $places[ $code ] ) && file_exists( $this->get_plugin_path() . '/towns/' . $code . '.php' ) ) {
                        include( $this->get_plugin_path() . '/towns/' . $code . '.php' );
                    }
                }
            }
